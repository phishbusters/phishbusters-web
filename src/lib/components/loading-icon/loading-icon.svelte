<script context="module" lang="ts">
	export interface LoadingIconProps extends HTMLOrSVGElement {
		className?: string;
		icon?:
			| 'audio'
			| 'ball-triangle'
...
<script lang="ts">
	...
	let className: LoadingIconProps['className'] = undefined;
	let icon: LoadingIconProps['icon'] = undefined;
	let color: LoadingIconProps['color'] = '#2d3748';

	export { className as class, icon, color };

	const iconColor = color || '#fff';
	// const iconColor = derived(darkModeStore, () => {
	// 	return !$darkModeStore.value ? color : '#ffffff';
	// });
</script>
...
<span {...$$props} class={className}>
	...
	{:else if icon == 'spinning-circles'}
		<svg width="20" viewBox="0 0 58 58" xmlns="http://www.w3.org/2000/svg" class="w-full h-full">
			<g fill="none" fill-rule="evenodd">
	...
	{:else if icon == 'tail-spin'}
		<svg width="20" viewBox="0 0 38 38" xmlns="http://www.w3.org/2000/svg" class="w-full h-full">
			<defs>
